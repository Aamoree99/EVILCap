    ...
    try {
        ...
        if (!messageExists) {
            ...
            try {
                ...
                if (response === '-') {
                    logAndSend('Пользователь ввёл "-", продолжаем создание сообщения.');
                    // Здесь можно продолжить создание сообщения или выполнить другие действия
                } else {
                    // Записать ID и завершить блок кода, если полученный ответ не "-"
                    const channelId = response;
                    logAndSend(`ID канала сохранён: ${channelId}`);
                    return; // Завершаем выполнение функции, так как ID получен
                }
            } catch (error) {
                logAndSend('Время ожидания ответа истекло.');
                return; // Завершаем выполнение функции, так как время ожидания истекло
            }

            const messageText = `В этом сообщении вы можете выбрать себе роль, тыкнув на соответствующую реакцию...`;
        ...
        logAndSend(`Произошла ошибка: ${error}`);
    }
}
